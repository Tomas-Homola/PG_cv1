...
void ImageViewer::on_pushButton_Draw_clicked()
{
	//infoMessage(QString("drawing " + ui->comboBox_SelectedFunction->currentText() + "\nGraph type: " + ui->comboBox_GraphType->currentText()));

	int graphType = ui->comboBox_GraphType->currentIndex();
	int interval = ui->spinBox_Interval->value();
	int axisSteps = ui->spinBox_xAxisDivision->value();

	clearImage();
	getCurrentViewerWidget()->drawAxes(axisSteps);

	if (ui->comboBox_SelectedFunction->currentIndex() == 0)
		getCurrentViewerWidget()->drawSin(graphType, interval, axisSteps);
	else
		getCurrentViewerWidget()->drawCos(graphType, interval, axisSteps);
}

void ImageViewer::on_pushButton_Erase_clicked()
{
	//warningMessage(QString("-nπ, nπ"));
	clearImage();
